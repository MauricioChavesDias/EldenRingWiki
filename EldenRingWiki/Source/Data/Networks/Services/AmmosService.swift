...
class AmmosService {
    ...
    private let endpointURL = "https://eldenring.fanapis.com/api/ammos"
    ...
    // MARK: - File Loader
    func loadAmmoFromFile(filename: String) -> AnyPublisher<Ammo, Error> {
        Future { promise in
    ...
    // MARK: - File Loader
    func loadAmmosFromFile(filename: String) -> AnyPublisher<[Ammo], Error> {
        Future { promise in
            DispatchQueue.global(qos: .background).async {
                guard let url = Bundle.main.url(forResource: filename, withExtension: "json") else {
                    return promise(.failure(URLError(.fileDoesNotExist)))
